...
accelerate launch --num_processes=$4 video-flow.py data.path="$1" log.exp_path="$2" log.comment="video_flow" \
...
accelerate launch --num_processes=$4 video-flow.py data.path="$1" log.exp_path="$2" log.comment="condiff" \
    branch.name="condiff" \
    training.n_iter=200_000 training.batchsize=128 training.lr=1e-4 training.accumulation_steps=$5 \
...
accelerate launch --num_processes=$4 video-flow.py data.path="$1" log.exp_path="$2" log.comment="video_biflow" \
